...
- __首次输入__：创建 Task，content 为用户输入原文（待总结状态）
- __主题确定__：Agent 首次响应后触发总结，确定任务主题
- __主题不变__：一旦确定，Task 主题不再变化
- __话题隔离__：无关话题通过子任务承接，避免污染主任务上下文
...
- __用户主动触发__：用户输入 `/btw` 创建子任务承接新话题
- __结束当前任务__：用户输入 `/finish` 结束当前任务，触发向父任务提交
