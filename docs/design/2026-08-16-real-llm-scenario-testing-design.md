...
- 场景首次运行后，将 Agent 使用的__工具集合__（去重、忽略调用次数）存为
  `tests/scenarios/golden/<scenario>.md`。最终输出文本__不进入__金标准比对——
  真实 LLM 输出措辞必然波动，字节全等只会产生噪音，文本正确性交由 Judge 语义判断。
- 后续运行自动 diff：仅比对工具集合（结构差异）——Agent 是否换了做事方式
  （不用工具 → 用工具、或换用别的工具）触发漂移并报告期望/实际集合；
  同一工具调用多次不视为回归。语义差异（输出内容）交 Judge 复核。
- 金标准更新必须显式覆盖（`--bless` 语义），防止静默漂移。
