...
- 完成时基于**当前** entries 重算分组，而非回放触发时的快照。摘要执行期间若有新
...
- 根因 2.3 的触发缺口（`Waiting(User)` 可触发压缩）**有意保留**：空闲任务超阈值时
